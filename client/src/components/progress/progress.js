...
export default class Progress extends PureComponent {
    ...
    createProgressComponents(length, completed) {
        ...
        for (let i = 0; i < length; i++) {
            if (i < completed) {
                result.push(<FontIcon key={i} className="material-icons" color={lightGreen500}>check_box</FontIcon>);
            } else {
                result.push(<FontIcon key={i} className="material-icons" color={grey500}>check_box_outline_blank</FontIcon>);
            }
        }
